...
class stratalMesh:
    ...
    def loadStratigraphy(self, timestep=0):
        ...
        layPoro = np.array((df['/layPoro']))
        depothick = np.array((df['/depothick']))
        ...
        self.ny = int((y.max() - y.min())/self.dx)
        self.nz = dep.shape[1]
        ...
        self.poro = poro.reshape((self.ny,self.nx,self.nz))
        self.depo = depo.reshape((self.ny,self.nx,self.nz))
    ...
    def buildMesh(self, outfolder='.'):
        ...
        d = np.zeros((self.nx, self.ny, self.nz))
        ...
        for k in range(self.nz):
            for j in range(self.ny):
                for i in range(self.nx):
                    ...
                    p[i,j,k] = self.poro[j,i,k]
                    d[i,j,k] = self.depo[j,i,k]
                    t[i,j,k] = k

        gridToVTK(vtkfile, x, y, z, pointData = {"relative elevation" : e, "thickness" :h, "depothick" :d, "layer ID" :t})
